...
The dataset shows that **`r round(adoption_summary %>% filter(practice_category == "Sustainable Practices") %>% pull(pct), 1)`%** of farmers have adopted sustainable practices, while **`r round(adoption_summary %>% filter(practice_category == "Complete Burning") %>% pull(pct), 1)`%** still engage in complete burning.
:::
